...
void tmr1_callback(OS_TMR *ptmr,void *p_arg) 
{
	...
	cpuusage+=OSCPUUsage;
	tcnt++;				    
 	LCD_ShowxNum(202,30,mymem_perused(SRAMIN)/10,3,16,0);	//显示内存使用率	 	  		 					    
...
void start_task2(void *pdata)
{
	...
	OSTaskSuspend(START2_TASK_PRIO); //挂起开始任务
}
...
void Third_Test(void)
{
	...
	OSInit();					//UCOS初始化
    OSTaskCreateExt((void(*)(void*) )start_task2,                //任务函数
                    (void*          )0,                         //传递给任务函数的参数
                    (OS_STK*        )&START2_TASK_STK[START2_STK_SIZE-1],//任务堆栈栈顶
                    (INT8U          )START2_TASK_PRIO,           //任务优先级
                    (INT16U         )START2_TASK_PRIO,           //任务ID，这里设置为和优先级一样
                    (OS_STK*        )&START2_TASK_STK[0],        //任务堆栈栈底
                    (INT32U         )START2_STK_SIZE,            //任务堆栈大小
                    (void*          )0,                         //用户补充的存储区
