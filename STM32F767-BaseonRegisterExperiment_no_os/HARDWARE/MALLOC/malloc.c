...
//前面 2MB用做 RGB LCD 屏的显存，不用于内存管理
//所以，用于内存管理的外部 SDRAM 内存池首地址为：0XC01F4000（0XC000 0000+1280*800*2） 
...
u32 mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((at(0x20000000+MEM3_MAX_SIZE)));	//内部TCM内存池MAP
