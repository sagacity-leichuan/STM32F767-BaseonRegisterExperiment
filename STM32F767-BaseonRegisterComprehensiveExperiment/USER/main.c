...
void system_init(void)
{
	...
	usart3_init(54,115200);		//初始化串口3波特率为115200
...
//开始硬件检测初始化
	...
	LCD_ShowString(5,ypos+fsize*j++,lcddev.width,lcddev.height,fsize, "CPU:STM32F767IGT6 216Mhz");
	LCD_ShowString(5,ypos+fsize*j++,lcddev.width,lcddev.height,fsize, "FLASH:1024KB  SRAM:512KB");	 
	...
	if(res==0X0D)//文件系统不存在
	{
		...
		res=f_mkfs("1:",FM_ANY,0,fatbuf,_MAX_SS);		//格式化SPI FLASH,1:,盘符;FM_ANY,自动选择文件系统类型,0,自动选择簇大小
		if(res==0)
		{
...
{	    
	...
	{			  
		...
		if(memshow_flag==1)
		{
			...
			pstcm=my_mem_perused(SRAMTCM);
			printf("in:%3.1f,ex:%3.1f,tcm:%3.1f\r\n",psin/10,psex/10,pstcm/10);//打印内存占用率
		}
	}
}
