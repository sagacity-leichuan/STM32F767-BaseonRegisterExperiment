...
//DMA2_Stream1中断服务回调函数
//处理硬件JPEG解码后输出的数据流
void jpeg_dma2s1_irq_callback(void)
{           
	JPEG->CR&=~(1<<12);			//关闭JPEG的DMA OUT
	JPEG->CR&=~(0X7E);			//关闭JPEG中断,防止被打断.
	jpeg_out_callback();		//执行输出回调函数
	JPEG->CR|=3<<5;				//使能EOC和HPD中断. 			 
}
//JPEG解码中断服务函数
...
extern void (*dma2s1_irq_callback)(void);	//DMA2 Stream1中断处理回调函数,在dcmi.c里面定义
...
{ 
	...
	dma2s1_irq_callback=jpeg_dma2s1_irq_callback;	//指向jpeg处理回调函数
