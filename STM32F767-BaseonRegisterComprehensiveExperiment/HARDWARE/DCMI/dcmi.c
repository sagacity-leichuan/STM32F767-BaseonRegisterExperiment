...
///////////////////////////////////////////////////////////////////////////////////
//dma2s1_irq_callback,在DMA2_Stream1_IRQHandler里面调用,处理DMA2 Stream1的中断.
//因为JPEG硬解和DCMI都用到DMA2_Stream1_IRQHandler这个函数,所以通过dma2s1_irq_callback
//来指向不同的服务函数,做不同的处理.

void (*dma2s1_irq_callback)(void);			//DMA2 Stream1中断处理回调函数

//DCMI DMA2 Stream1中断处理回调函数
void dcmi_dma2s1_irq_callback(void)
{
...
{
	u32 tempreg=0;
	dma2s1_irq_callback=dcmi_dma2s1_irq_callback;//指向dcmi处理回调函数
	//设置IO 
