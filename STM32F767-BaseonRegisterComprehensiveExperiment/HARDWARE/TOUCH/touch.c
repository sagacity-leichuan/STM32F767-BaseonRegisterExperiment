...
{								 
	...
	while(1)//如果连续10秒钟没有按下,则自动退出
	{
		...
		if((tp_dev.sta&(TP_PRES_DOWN|TP_CATH_PRES))==TP_CATH_PRES)//按键按下了一次(此时按键松开了.)
		{	
			outtime=0;		
...
{	
	...
	}else if(lcddev.id==0X1963||lcddev.id==0X7084||lcddev.id==0X7016)//SSD1963 7寸屏或者 7寸800*480/1024*600 RGB屏
	{
		...
		tp_dev.touchtype|=lcddev.dir&0X01;//横屏还是竖屏 
		return 0;
	}else if(lcddev.id==0X8016)			//8寸 1024*600 RGB屏
	{
		CY8CTMA_Init();
		tp_dev.scan=CY8CTMA_Scan;		//扫描函数指向GT928触摸屏扫描
		tp_dev.touchtype|=0X80;			//电容屏 
		tp_dev.touchtype|=lcddev.dir&0X01;//横屏还是竖屏 
		return 0;
	}else if(lcddev.id==0X1016)			//10.1寸 1024*600 RGB屏
	{
		GT928_Init();
		tp_dev.scan=GT928_Scan;    	//扫描函数指向GT928触摸屏扫描
		tp_dev.touchtype|=0X80;		//电容屏 
		tp_dev.touchtype|=lcddev.dir&0X01;//横屏还是竖屏 
		return 0;
	}else
