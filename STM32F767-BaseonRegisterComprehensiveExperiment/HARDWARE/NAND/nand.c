...
u8 NAND_Init(void)
{
	...
	tempreg|=5<<9;				//设置TCLR(tCLR=CLE到RE的延时)=(TCLR+TSET+2)*THCLK,THCLK=1/216=4.6ns,对应tCLR,最少10ns
	tempreg|=5<<13;				//设置TAR(tAR=ALE到RE的延时)=(TAR+TSET+2)*THCLK,THCLK=1/216=4.6ns,对应tAR,最少10ns
	FMC_Bank3->PCR=tempreg;		//设置NAND/PC卡控制寄存器

	tempreg=1<<0;				//MEMSET,建立时间2个HCLK(设置值+1),对应tREH/tWH,最少7ns
	tempreg|=3<<8;				//MEMWAIT,等待时间4个HCLK(设置值+1),对应tRP/tWP,最少10ns
	tempreg|=1<<16;				//MEMHOLD,保持时间1个HCLK,对应tRHZ/tDH,最少5ns
	tempreg|=1<<24;				//MEMHIZ,高阻态时2个HCLK(设置值+1),设置等于MEMSET的值即可
