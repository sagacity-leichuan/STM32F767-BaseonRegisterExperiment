...
//横屏时,最高支持1024*600的视频,10帧,中等质量,22.05K音频的播放.
//
////////////////////////////////////////////////////////////////////////////////// 	   
...
u8 mjpeg_rgb_fastdec=0;			//RGB屏快速显示,仅在使用RGB屏的时候,且是横屏,且分辨率为整个屏幕分辨率的时候,才使用
								//0,不使用
								//1,使用
...
u8 mjpeg_init(u16 offx,u16 offy,u32 width,u32 height)
{
	...
	mjpeg_rgb_fastdec=0;						//RGB屏非快速显示
	if(lcdltdc.pwidth!=0)						//RGB屏
	{
		if(imgoffx==0&&imgoffy==0)			
		{
			mjpeg_rgb_fastdec=1;				//符合横屏,且整屏显示的要求,快速显示
		}
	}
	return 0; 
}
...
{ 
	...
	while(1)
	{
		...
		{
			if(mjpeg_rgb_fastdec)mcublkindex+=mjpeg.ycbcr2rgb(mjpeg.outbuf[mjpeg.outbuf_read_ptr].buf,(u8*)mjpeg_rgb_framebuf,mcublkindex,mjpeg.outbuf[mjpeg.outbuf_read_ptr].size);//RGB横屏
			else mcublkindex+=mjpeg.ycbcr2rgb(mjpeg.outbuf[mjpeg.outbuf_read_ptr].buf,(u8*)rgb565buf,mcublkindex,mjpeg.outbuf[mjpeg.outbuf_read_ptr].size);//MCU屏/RGB竖屏
	...
	{
		if(mjpeg_rgb_fastdec==0)mjpeg_fill_color(imgoffx,imgoffy,mjpeg.Conf.ImageWidth,mjpeg.Conf.ImageHeight,rgb565buf);//MCU屏/RGB竖屏  
	} 
	return 0;
}
