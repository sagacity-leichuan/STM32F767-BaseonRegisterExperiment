...
{ 
	...
	{ 
		...
		if(res==FR_OK)
		{
			...
			if(fc->min_blocksize==fc->max_blocksize&&fc->max_blocksize!=0)//必须min_blocksize等于max_blocksize
			{
				...
				buffer=mymalloc(SRAMTCM,fc->max_framesize); 	//申请解码帧缓存 
				decbuf0=mymalloc(SRAMTCM,fc->max_blocksize*4);
				decbuf1=mymalloc(SRAMTCM,fc->max_blocksize*4);
			}else res+=1;//不支持的音频格式  
		}
	}
	...
	myfree(SRAMTCM,buffer);
	myfree(SRAMTCM,decbuf0);
	myfree(SRAMTCM,decbuf1); 
    return res;
} 
