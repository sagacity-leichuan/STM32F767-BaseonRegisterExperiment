...
void sms_sram_free(void)
{
	...
	myfree(SRAMTCM,S_RAM);
	myfree(SRAMTCM,SMS_VDP);
	myfree(SRAMTCM,psg_buffer);
	myfree(SRAMTCM,SN76496);
	myfree(SRAMTCM,Z80A);
	myfree(SRAMTCM,VRam);
...
{ 
	...
	S_RAM=mymalloc(SRAMTCM,0X4000);				//申请16K字节
 	SMS_VDP=mymalloc(SRAMTCM,sizeof(SVDP));		
	psg_buffer=mymalloc(SRAMTCM,SNBUF_size*2);	//申请内存SNBUF_size
	SN76496=mymalloc(SRAMTCM,sizeof(t_SN76496));//申请内存184
	Z80A=mymalloc(SRAMTCM,sizeof(CPU80));		//申请内存 
	VRam=mymalloc(SRAMTCM,0x4000); 				//申请16K字节
